...
#		l_likelihood[config] = np.sum(P_gamma[config,:])
	l_likelihood[config] = np.sum(data)
...
evidence = np.ma.array(l_evidence, mask=False)
...
#print(np.array(sorted_odds).shape)
#print(np.array(sorted_binaries[1]).shape)
#print(sorted_binaries)
 
plot_functions.plot_data(all_data[0], output)
