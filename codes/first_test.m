...
%             prob = prob * prior; 
        ...
        if bit == '1'
            each_h_1 = (1/(sqrt(2*pi)*sigma))*exp(-((D-h_vals).^2)/(2*sigma^2));
            P_gamma = sum(each_h_1);
            bit1 = 1;
        end
        if bit == '0'
            P_gamma = (1/(sqrt(2*pi)*sigma))*exp(-((D).^2)/(2*sigma^2));
%             P_gamma = sum(each_h_0);
