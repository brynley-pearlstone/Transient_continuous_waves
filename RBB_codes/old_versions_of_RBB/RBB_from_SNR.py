...
for index in range(len(l_evidence)):
	...
	for i in range(len(evidence)):
                if i != index:
                        running_evidence_sum =  np.logaddexp(running_evidence_sum, evidence[i])    #odds_all_denom = np.sum(mask_evidence)
...
plot_functions.barcode_plot(sorted_binaries,  sorted_odds, all_data[running_total],  output)
