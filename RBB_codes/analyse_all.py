...
#	print(item[0:2])
	os.system('lalapps_pulsar_parameter_estimation_nested --detectors H1,L1 --par-file ' + par_path + ' --input-files H1/' + (H1_chunks[item]) + ',L1/' + (L1_chunks[item]) + ' --outfile output/' + (H1_chunks[item][29:-3]) + 'hdf --Nlive 1024 --Nmcmcinitial 0 --tolerance 0.1 --prior-file ' + prior_path )
